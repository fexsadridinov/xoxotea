...
ink = HexColor('#202420')
for lang in __import__('os').environ.get('XOXO_PDF_LOCALES', 'uk,en,ru,zh').split(','):
    chinese = lang == 'zh'
    if chinese: pdfmetrics.registerFont(TTFont('Chinese', str(root / 'src/app/fonts/xoxo-sc.ttf')))
    sans_font = 'Chinese' if chinese else 'Sans'
    mono_font = 'Chinese' if chinese else 'Mono'
    ...
    def txt(text, x, y, size=10, font='Sans'):
        f.setFont(mono_font if font == 'Mono' else sans_font, size); f.drawString(x, y, text)
    def paragraph(text, size=8, leading=None):
        return Paragraph(escape(text.replace('，货币符号为 ₴','') if chinese else text), ParagraphStyle('p', fontName=sans_font, wordWrap='CJK' if chinese else None, fontSize=size, leading=leading or size*1.4, textColor=ink))
    ...
    econ=c['economics']
    headers=[c['economicsExtra']['scenario'],econ['revenue']+(' / UAH' if chinese else ' / ₴'),'EBITDA / '+('UAH' if chinese else '₴'),econ['payback']]
    rows=[[paragraph(t,8) for t in headers]]
    for i,v in enumerate(results):
        money=lambda n: format(round(n),',') if chinese else format(round(n),',').replace(',',' ')
        payback=format(v['payback'],'.1f').replace('.',',' if lang not in ['en','zh'] else '.')
    ...
    table.setStyle(TableStyle([('FONTNAME',(0,0),(-1,-1),mono_font),('FONTSIZE',(0,0),(-1,-1),9),('TEXTCOLOR',(0,0),(-1,-1),ink),('VALIGN',(0,0),(-1,-1),'TOP'),('LEFTPADDING',(0,0),(-1,-1),0),('RIGHTPADDING',(0,0),(-1,-1),12),('TOPPADDING',(0,0),(-1,-1),5),('BOTTOMPADDING',(0,0),(-1,-1),6),('LINEBELOW',(0,0),(-1,0),.4,HexColor('#a5afa5'))]))
    _,h=table.wrap(515,1000); table.drawOn(f,40,y-h); y-=h+12
    y=para(econ['assumptions'],y,7.5)-12
    if chinese: y=para(econ['explanation'],y,7.5)-12
