...
content={l:json.loads((root/f'content/{l}.json').read_text()) for l in ['uk','en','ru','zh']}
def shape(v):
    if isinstance(v,dict):return {k:shape(x) for k,x in v.items()}
    if isinstance(v,list):return [shape(x) for x in v]
    return type(v).__name__
assert shape(content['uk'])==shape(content['en'])==shape(content['ru'])==shape(content['zh'])
...
for l in content:
    for view in ['', 'menu', 'investors']:
        ...
        assert page.lang==({'zh':'zh-Hans'}.get(l,l)) and page.h1==1,(folder,page.lang,page.h1)
...
print('PASS: translation key parity, 12 localized HTML documents, headings, IDs, links and media targets; 404 generated.')
